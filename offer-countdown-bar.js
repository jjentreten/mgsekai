/**
 * Barra do timer de oferta ativada - aparece em todas as páginas da loja
 * (exceto checkout/pix, onde a barra não existe no HTML).
 * Lê localStorage e exibe/atualiza o countdown quando a oferta está ativa.
 * Ao expirar: popup "Sua promoção acabou... Iremos te dar mais 5 minutos!"
 */
(function () {
  ...
  function showExpiredPopup(onAccept, onRefuse) {
    ...
    popup.innerHTML = `
      <div class="popup-expired__backdrop"></div>
      <div class="popup-expired__box" role="dialog" aria-label="Sua promoção acabou" aria-modal="true">
        <p class="popup-expired__message">Oh não! Sua promoção acabou... :(</p>
        <p class="popup-expired__message popup-expired__message--sub">Iremos te dar mais 5 minutos!</p>
        <div class="popup-expired__actions">
          <button type="button" class="popup-expired__btn popup-expired__btn--accept">Ok</button>
          <button type="button" class="popup-expired__btn popup-expired__btn--refuse">Não, obrigado</button>
        </div>
      </div>
    `;
    ...
    btnRefuse.addEventListener('click', function () {
      finish();
      if (typeof onRefuse === 'function') onRefuse();
    });
